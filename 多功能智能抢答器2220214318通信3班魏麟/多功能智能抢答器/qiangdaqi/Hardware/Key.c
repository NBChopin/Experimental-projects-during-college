...
void Key_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15 | GPIO_Pin_0 | GPIO_Pin_11;
...
uint8_t Reset_GetNum(void)
{
	uint8_t ResetNum = 0;		//定义变量，默认键码值为0
	...
	return ResetNum;			//返回键码值，如果没有按键按下，所有if都不成立，则键码为默认值0
}

uint8_t Start_GetNum(void)
{
	uint8_t StartNum = 0;		//定义变量，默认键码值为0
	...
	return StartNum;			//返回键码值，如果没有按键按下，所有if都不成立，则键码为默认值0
}
